...
public class Paging2 {
	...
	public Paging2(int totalCount) {
		setTotalCount(totalCount);
		
		makePaging();
	}
	
	public Paging2(int totalCount, int curPage, int listCount) {
		setTotalCount(totalCount);
		setCurPage(curPage);
	...
	private void makePaging() {
		...
		if(listCount == 0)	this.listCount = 5; //화면에 보여질 게시글 수를 5개로 설정
		if(pageCount == 0)	this.pageCount = 5; //화면에 보여질 페이징 수를 5개로 설정
