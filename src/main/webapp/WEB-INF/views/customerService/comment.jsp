...
$(document).ready(function() {
	...
	function getCommentList(){
		$.ajax({
			url : "/customerService/comment?inum=${viewInquire.inum}"
			, type : "get"
			...
			, success : function(res){
				...
				if(res.list.length > 0){
					...
					for(i = 0; i < list.length; i++){
						...
						output += "<span id='ajaxRecontent"+list[i].incno+"'>" + list[i].incommContent +"</span>";
		...
		if(cf){
			...
			if(!isCreate){
				...
				$.ajax({
					...
					, type : "POST"
					, data : { 
						"incommContent" : content.value, "bno" : ${viewBoard.bno},"adminid" :  adminid, "parentNo" :  0}
					, dataType : "text"
					, success : function(data){
						if(data == "success"){        
							console.log("댓글 등록 완료");
